 ...
 * NEON-accelerated: processes 8 pixels at a time, same as rgbToNv21.
 */
JNIEXPORT void JNICALL
Java_com_briefplantrain_virtucam_NativeEncoder_rgbToI420(
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (; x <= width - 8; x += 8) {
            ...
            uint16x8_t y16 = vmlaq_n_u16(
                vmlaq_n_u16(vmulq_n_u16(r16, 66), g16, 129),
                b16, 25
            );
            y16 = vaddq_u16(y16, vdupq_n_u16(128));
            y16 = vshrq_n_u16(y16, 8);
            ...
            vst1_u8((uint8_t*)&i420[idx], vqmovn_u16(y16));
        }
    ...
    // NEON-accelerated U and V planes (planar, separate buffers)
    int uvIdx = 0;
    for (int y = 0; y < height; y += 2) {
        int x = 0;
        ...
        for (; x <= width - 8; x += 8) {
            ...
            // Store to separate U and V planes (I420 planar format)
            for (int i = 0; i < 4 && uvIdx + i < frameSize / 4; i++) {
                i420[uOffset + uvIdx + i] = static_cast<jbyte>(uVals[i]);
                i420[vOffset + uvIdx + i] = static_cast<jbyte>(vVals[i]);
            }
            uvIdx += 4;
        }
    ...
    // Scalar fallback — U and V planes (planar)
    int uvIdx = 0;
    for (int y = 0; y < height; y += 2) {
        for (int x = 0; x < width; x += 2) {
            int idx = y * width + x;
