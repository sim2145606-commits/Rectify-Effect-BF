...
**Core Issue**: When LSPosed activates a module, it only loads the module into the **target apps** (apps in the scope), not into the module's own app. The VirtuCam app itself is not being hooked, so the marker file may not exist or may be stale when checking.
